...
async function exportPdf(teacher:Profile,evs:Evidence[],tProfile:any) {
  ...
  const secHtml=grouped.map(({sec,evs:sevs})=>`
    ...
    ${sevs.map(ev=>`<div class="ev"><div class="ev-hd"><span>${ev.type==='image'?'🖼️':ev.type==='pdf'?'📄':ev.type==='link'?'🔗':'📋'}</span><strong>${ev.title}</strong>${ev.date?`<span class="badge">${ev.date}</span>`:''}</div>${ev.desc?`<div class="ev-desc">${ev.desc}</div>`:''}${(ev as any).impact?`<div class="ev-impact">📈 ${(ev as any).impact.label}: ${(ev as any).impact.pre}% → ${(ev as any).impact.post}%</div>`:''}${ev.sup_rating>0?`<div style="color:#f59e0b;font-size:12px;margin-top:4px">${'⭐'.repeat(ev.sup_rating)} ${ev.note||''}</div>`:''}</div>`).join('')}</div>`).join('')
