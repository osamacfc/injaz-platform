...
export default function DashboardPage() {
  ...
  const filtered=useMemo(()=>{
    ...
    if(search.trim()) p=p.filter(x=>(x.full_name||'').includes(search.trim()))
    return p
                ...
                <div key={p.id} className="glass2 anim-up" style={{cursor:'pointer',transition:'all .2s',padding:18}}
                  ...
                  <div style={{display:'flex',gap:12,alignItems:'center',marginBottom:14}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:700,fontSize:14,color:'var(--tx)',marginBottom:2,lineHeight:1.3}}>{p.full_name}</div>
            ...
            {filtered.map((p,i)=>{
              ...
              return(
                <div key={p.id} className="glass2 anim-up" style={{cursor:'pointer',padding:'14px 18px',display:'flex',alignItems:'center',gap:14,transition:'all .2s'}}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontWeight:700,fontSize:14,color:'var(--tx)'}}>{p.full_name}</div>
